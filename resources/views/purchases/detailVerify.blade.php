      							...
      							<input type="hidden" name="_method" value="PUT">
										...
										<div class="rown">
											...
											<div class="col-xs-6 text-right">
												<button type="submit" {{ (\Auth::user()->checkAccessByIdForCorp(request()->corpID, 58, 'E') || \Auth::user()->checkAccessByIdForCorp(request()->corpID, 58, 'D')) ? '' : 'disabled' }} class="btn btn-danger delete_request_verify" value="" name="delete_request">Delete Request</button>
												<button type="button" {{ (\Auth::user()->checkAccessByIdForCorp(request()->corpID, 58, 'E') || \Auth::user()->checkAccessByIdForCorp(request()->corpID, 58, 'D')) ? '' : 'disabled' }} class="btn btn-primary btn-verify" >Verify</button>
											</div>
										</div>
										</form>
									</div>
