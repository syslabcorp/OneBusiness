...
$(document).ready(function () {
    ...
    $('body').on('change', '.changePageCompany', function (event) {
        var search = location.search.replace(/&corpID=[0-9]*/g, '');
        search += (search ? '&corpID=' : '?corpID=') + event.target.value;

        window.location = location.pathname + search;
    });

    $('.tooltipster').tooltipster(_defineProperty({
        animation: 'fade',
        delay: 200,
 ...
 */
showConfirmMessage = function showConfirmMessage(message) {
  var title = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "Alert";
