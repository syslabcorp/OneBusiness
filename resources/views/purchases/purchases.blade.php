  ...
  <table class="table table-bordered table-striped">
    ...
    <tbody>
      ...
      <tr class="purchaseRow" style="" >
        ...
        <td><input type="checkbox" class="form-check-input" name="purchase[{{ $loop->index+1 }}][eqp]" value="{{ $row->eqp == 1 ? '1' : '0' }}"  {{ $row->eqp == 1 ? 'checked' : '' }}></td>
        <td><input type="checkbox" class="form-check-input" name="purchase[{{ $loop->index+1 }}][prt]" value="{{ $row->prt == 1 ? '1' : '0' }}" {{ $row->prt == 1 ? 'checked' : '' }}></td>
        <td>
        <select name="purchase[{{ $loop->index+1 }}][item_name]" class="form-control">
        @if(count($branches))
        ...
        <td>
          <input type="number" class="form-control text-center label-table-min quantity" name="purchase[{{ $loop->index+1 }}][qty_to_order]" value="{{ $row->qty_to_order }}" autocomplete="off">
        </td>
